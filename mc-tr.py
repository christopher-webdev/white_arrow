# ===========================
# train_stack_part1.py
# ===========================
# Part 1/2 of the stacking pipeline:
# - Loads test-combined.csv (from your labeler)
# - Trains 3 multiclass classifiers: y_1.2R, y_2.2R, y_3.2R  (classes: loss/neutral/win -> 0/1/2)
# - Trains a regressor for rr_label
# - Uses StratifiedKFold OOF for the 3 classifiers and KFold OOF for regression
# - Creates META TRAIN FEATURES = [OOF probs (all 3) + OOF reg + FULL ORIGINAL FEATURES]
# - Creates META TEST FEATURES  = [TEST probs (avg folds) + TEST reg (avg folds) + FULL ORIGINAL FEATURES]
# - Saves everything to artifacts_stack_base_<meta_target>_<model_lib>/
#
# Then run Part 2 to train the meta model and evaluate.
#
...
# -----------------------------
DEFAULT_DATA_PATH = "test-combined.csv"
DEFAULT_META_TARGET = "y_2.2R"  # meta will ultimately predict this 3-class target
DEFAULT_MODEL_LIB = "lgbm"      # "lgbm" or "xgb"
N_SPLITS = 5
RANDOM_STATE = 42
N_TRIALS = 40           # per base model; increase later if you want
TEST_SIZE = 0.20
MAX_EARLY_STOP = 200
BASE_TARGETS = ["y_1.2R", "y_2.2R", "y_3.2R"]  # first-layer classifiers
...
def pick_feature_columns(df, extra_exclude):
    # Keep numeric columns that are not labels/targets/obvious leakage
    exclude = set(extra_exclude) | {
        "label", "pair",
        "entry_price", "stop_loss_price", "stop_loss_distance", "sl_ratio_to_entry",
        "Time", "time", "timestamp", "date", "datetime"
    }
    feats = [c for c in df.columns if c not in exclude and pd.api.types.is_numeric_dtype(df[c])]
    return feats

def map_3class_to012(y_series):
    # Expect values -1, 0, 1 -> map to 0,1,2
    mapping = {-1: 0, 0: 1, 1: 2}
...
def plot_cm(cm, classes, title, outpath):
    ...
    ax.set_xticks(tick); ax.set_xticklabels(classes, rotation=45)
...
# Tuning (Optuna)
# -------------------------------------------------
def tune_lightgbm_mc(X, y, Xv, yv, n_trials=N_TRIALS):
...
def tune_xgboost_mc(X, y, Xv, yv, n_trials=N_TRIALS):
...
def tune_lightgbm_reg(X, y, Xv, yv, n_trials=N_TRIALS):
    import lightgbm as lgb
    def obj(trial):
        params = {
        ...
        model = lgb.LGBMRegressor(**params, n_estimators=5000, random_state=RANDOM_STATE, n_jobs=-1)
        model.fit(
            X, y,
            eval_set=[(Xv, yv)],
            eval_metric="rmse",
...
def tune_xgboost_reg(X, y, Xv, yv, n_trials=N_TRIALS):
    import xgboost as xgb
    def obj(trial):
        params = {
        ...
        model = xgb.XGBRegressor(**params, n_estimators=5000, random_state=RANDOM_STATE, nthread=-1)
        model.fit(X, y, eval_set=[(Xv, yv)], verbose=False, early_stopping_rounds=MAX_EARLY_STOP)
        pred = model.predict(Xv)
        return float(np.sqrt(np.mean((pred - yv) ** 2)))
    study = optuna.create_study(direction="minimize")
...
# -------------------------------------------------
# Fit helpers (use tuned params)
# -------------------------------------------------
def fit_mc(model_lib, params, X, y, Xv=None, yv=None):
    if model_lib == "lgbm":
        import lightgbm as lgb
        mdl = lgb.LGBMClassifier(**params, n_estimators=5000, random_state=RANDOM_STATE, n_jobs=-1)
        mdl.fit(X, y,
        ...
        import xgboost as xgb
        mdl = xgb.XGBClassifier(**params, n_estimators=5000, random_state=RANDOM_STATE, nthread=-1)
        mdl.fit(X, y,
                eval_set=[(Xv, yv)] if Xv is not None else None,
                verbose=False,
...
def fit_reg(model_lib, params, X, y, Xv=None, yv=None):
    if model_lib == "lgbm":
        import lightgbm as lgb
        mdl = lgb.LGBMRegressor(**params, n_estimators=5000, random_state=RANDOM_STATE, n_jobs=-1)
        mdl.fit(X, y,
                eval_set=[(Xv, yv)] if Xv is not None else None,
                eval_metric="rmse",
                callbacks=[lgb.early_stopping(stopping_rounds=MAX_EARLY_STOP, verbose=False)] if Xv is not None else None)
        ...
        import xgboost as xgb
        mdl = xgb.XGBRegressor(**params, n_estimators=5000, random_state=RANDOM_STATE, nthread=-1)
        mdl.fit(X, y,
                eval_set=[(Xv, yv)] if Xv is not None else None,
                verbose=False,
                early_stopping_rounds=MAX_EARLY_STOP if Xv is not None else None)
...
# -------------------------------------------------
# OOF for one multiclass target
# -------------------------------------------------
def oof_for_multiclass_target(model_lib, X_train, y_train, X_test, target_name, art_dir):
    # Tune once on a split of training
    Xtr, Xva, ytr, yva = train_test_split(X_train, y_train, test_size=0.2, stratify=y_train, random_state=RANDOM_STATE)
    if model_lib == "lgbm":
        best = tune_lightgbm_mc(Xtr, ytr, Xva, yva)
    else:
        best = tune_xgboost_mc(Xtr, ytr, Xva, yva)

    # OOF with tuned params
    skf = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE)
    oof_pred = np.zeros((len(X_train), 3), dtype=float)
    test_pred_folds = np.zeros((len(X_test), 3, N_SPLITS), dtype=float)
    models = []

    for k, (idx_tr, idx_va) in enumerate(skf.split(X_train, y_train), 1):
        X_tr, y_tr = X_train.iloc[idx_tr], y_train.iloc[idx_tr]
        X_va, y_va = X_train.iloc[idx_va], y_train.iloc[idx_va]

        mdl = fit_mc(model_lib, best, X_tr, y_tr, X_va, y_va)
        oof_pred[idx_va] = mdl.predict_proba(X_va)
        test_pred_folds[:, :, k-1] = mdl.predict_proba(X_test)
        models.append(mdl)
        print(f"[{target_name}] fold {k} done")

    test_pred = np.mean(test_pred_folds, axis=2)

    # Base OOF/test metrics (for info)
    oof_cls = np.argmax(oof_pred, axis=1)
    oof_metrics = {
        "accuracy": float(accuracy_score(y_train, oof_cls)),
        "macro_f1": float(f1_score(y_train, oof_cls, average="macro")),
        "logloss": float(log_loss(y_train, oof_pred, labels=[0,1,2])),
        "report": classification_report(y_train, oof_cls, target_names=["loss","neutral","win"])
    }
    with open(os.path.join(art_dir, f"{target_name}_base_oof_metrics.json"), "w") as f:
        json.dump(oof_metrics, f, indent=2)

    pd.DataFrame(oof_pred, columns=[f"{target_name}_proba_loss", f"{target_name}_proba_neutral", f"{target_name}_proba_win"])\
      .to_csv(os.path.join(art_dir, f"{target_name}_oof_pred.csv"), index=False)
    pd.DataFrame(test_pred, columns=[f"{target_name}_proba_loss", f"{target_name}_proba_neutral", f"{target_name}_proba_win"])\
      .to_csv(os.path.join(art_dir, f"{target_name}_test_pred.csv"), index=False)

    return best, models, oof_pred, test_pred

# -------------------------------------------------
# OOF for regression (rr_label)
# -------------------------------------------------
def oof_for_regression(model_lib, X_train, y_train, X_test, art_dir):
    # Tune
    Xtr, Xva, ytr, yva = train_test_split(X_train, y_train, test_size=0.2, random_state=RANDOM_STATE)
    if model_lib == "lgbm":
        best = tune_lightgbm_reg(Xtr, ytr, Xva, yva)
    else:
        best = tune_xgboost_reg(Xtr, ytr, Xva, yva)

    # KFold OOF
    kf = KFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE)
    oof_pred = np.zeros(len(X_train), dtype=float)
    test_pred_folds = np.zeros((len(X_test), N_SPLITS), dtype=float)
    models = []

    for k, (idx_tr, idx_va) in enumerate(kf.split(X_train), 1):
        X_tr, y_tr = X_train.iloc[idx_tr], y_train.iloc[idx_tr]
        X_va, y_va = X_train.iloc[idx_va], y_train.iloc[idx_va]

        mdl = fit_reg(model_lib, best, X_tr, y_tr, X_va, y_va)
        oof_pred[idx_va] = mdl.predict(X_va)
        test_pred_folds[:, k-1] = mdl.predict(X_test)
        models.append(mdl)
        print(f"[rr_label] fold {k} done")

    test_pred = np.mean(test_pred_folds, axis=1)

    # Save
    pd.DataFrame({"rr_oof_pred": oof_pred}).to_csv(os.path.join(art_dir, "rr_oof_pred.csv"), index=False)
    pd.DataFrame({"rr_test_pred": test_pred}).to_csv(os.path.join(art_dir, "rr_test_pred.csv"), index=False)

    # Basic regression OOF metrics
    rmse = float(np.sqrt(np.mean((oof_pred - y_train.values) ** 2)))
    r2 = float(1 - np.sum((oof_pred - y_train.values) ** 2) / np.sum((y_train.values - y_train.values.mean()) ** 2))
    with open(os.path.join(art_dir, "rr_base_oof_metrics.json"), "w") as f:
        json.dump({"rmse": rmse, "r2": r2}, f, indent=2)

    return best, models, oof_pred, test_pred

# -------------------------------------------------
# Main (build meta features)
# -------------------------------------------------
def main(data_path, meta_target, model_lib):
    assert model_lib in ("lgbm", "xgb")
    df = pd.read_csv(data_path)

    # Require all base targets + rr_label present (drop NaNs)
    needed = BASE_TARGETS + ["rr_label"]
    df_model = df.dropna(subset=needed).copy()

    if meta_target not in df_model.columns:
        raise KeyError(f"{meta_target} not in data.")

    # Map 3-class to 0/1/2 for each base target
    y_cols012 = {}
    for t in BASE_TARGETS:
        if t not in df_model.columns:
            raise KeyError(f"Missing base target {t}")
        y_cols012[t] = map_3class_to012(df_model[t])

    # Feature set (exclude labels/rr_label)
    exclude_cols = set(needed + BASE_TARGETS)
    feature_cols = pick_feature_columns(df_model, extra_exclude=list(exclude_cols))
    if not feature_cols:
        raise RuntimeError("No numeric feature columns found after exclusions. Check your dataset/features.")
    ...
    # Train/test split (consistent split used by all base models)
    y_meta012 = map_3class_to012(df_model[meta_target])
    X_train, X_test, y_train_meta, y_test_meta = train_test_split(
        X_all, y_meta012, test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=y_meta012
    )

    art_dir = f"artifacts_stack_base_{meta_target}_{model_lib}"
    ensure_dir(art_dir)

    # Save feature list & split indices
    with open(os.path.join(art_dir, "feature_cols.json"), "w") as f:
        json.dump(feature_cols, f, indent=2)

    # --- First layer: 3 classifiers OOF + test preds
    base_oof_blocks = []
    base_test_blocks = []
    summary_meta = {}
    ...
    for t in BASE_TARGETS:
        y_all = y_cols012[t]
        # Align y with the same index/order as X_all
        y_all = y_all.loc[X_all.index]
        y_tr = y_all.loc[X_train.index]
        y_te = y_all.loc[X_test.index]

        best_params, models, oof_pred, test_pred = oof_for_multiclass_target(
            model_lib, X_train, y_tr, X_test, t, art_dir
        )

        # Collect blocks with proper column names
        oof_df = pd.DataFrame(oof_pred, columns=[f"{t}_proba_loss", f"{t}_proba_neutral", f"{t}_proba_win"], index=X_train.index)
        tst_df = pd.DataFrame(test_pred, columns=[f"{t}_proba_loss", f"{t}_proba_neutral", f"{t}_proba_win"], index=X_test.index)
        base_oof_blocks.append(oof_df)
        base_test_blocks.append(tst_df)

        summary_meta[t] = {"best_params": best_params}

    # --- Regression (rr_label)
    rr_all = df_model["rr_label"].astype(float).loc[X_all.index]
    rr_tr = rr_all.loc[X_train.index]
    rr_te = rr_all.loc[X_test.index]

    best_rr, rr_models, rr_oof, rr_test = oof_for_regression(model_lib, X_train, rr_tr, X_test, art_dir)
    rr_oof_df = pd.DataFrame({"rr_oof_pred": rr_oof}, index=X_train.index)
    rr_tst_df = pd.DataFrame({"rr_test_pred": rr_test}, index=X_test.index)

    # --- Build META features: [OOF probs (all 3) + rr_oof + FULL FEATURES]
    meta_train = pd.concat(base_oof_blocks + [rr_oof_df, X_train], axis=1)
    meta_test  = pd.concat(base_test_blocks + [rr_tst_df, X_test], axis=1)

    # Save meta frames + y for meta
    meta_train.to_csv(os.path.join(art_dir, "meta_train_features.csv"))
    meta_test.to_csv(os.path.join(art_dir, "meta_test_features.csv"))
    pd.DataFrame({"y_train_meta": y_train_meta}, index=X_train.index)\
      .to_csv(os.path.join(art_dir, "y_train_meta.csv"))
    pd.DataFrame({"y_test_meta": y_test_meta}, index=X_test.index)\
      .to_csv(os.path.join(art_dir, "y_test_meta.csv"))

    # Also save a compact summary
    with open(os.path.join(art_dir, "base_summary.json"), "w") as f:
        json.dump(summary_meta, f, indent=2)

    print("\n✅ Part 1 done.")
    print(f"Meta-train shape: {meta_train.shape} | Meta-test shape: {meta_test.shape}")
    print(f"Artifacts saved to: {art_dir}")
    print("Next: run Part 2 to train the meta model and evaluate.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--data", default=DEFAULT_DATA_PATH, type=str, help="Path to test-combined.csv")
    parser.add_argument("--meta_target", default=DEFAULT_META_TARGET, type=str, help="Which y_*R the meta should learn (3-class)")
    parser.add_argument("--model", default=DEFAULT_MODEL_LIB, type=str, help="'lgbm' or 'xgb' for base models")
    args = parser.parse_args()
    main(args.data, args.meta_target, args.model)
