...
# ────────────── STATISTICAL FEATURE ENGINE ──────────────
    ...
    def detect_double_top_bottom(df, distance=5, threshold=0.002):
...
def filter_ml_trades_buy(df, file, rr_cap=3, max_lookahead=48):
    EPS = 1e-9
        ...
        if prev_close < prev_sma9 and curr_close > curr_sma9 and valid_hour and next_high: #  :  
            ...
            for j in range(1, max_lookahead + 1):
                ...
                if future['Low'] <= sl_price:
                    ...
                    time_to_outcome = j
                    break
                rr = (future['Close'] - entry_price) / sl_dist
                if rr >= rr_cap:
                    rr_achieved = rr_cap
                    hit_type = 1  # TP
                    time_to_outcome = j
...
def run_on_multiple_csvs(csv_files, rr_cap=3, max_lookahead=48):
    ...
    for file in csv_files:
        ...
        try:
            df = pd.read_csv(file)
            trades_df = filter_ml_trades_buy(df, file, rr_cap=rr_cap, max_lookahead=max_lookahead)
    ...
    if all_trades:
        ...
        combined.to_csv("ggtx.csv", index=False)
...
# List of your CSVs
csv_files = ["gbp.csv",]  #,"xau.csv",  "jpy.csv", "cad.csv" You can add more here,
